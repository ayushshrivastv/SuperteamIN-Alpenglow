...
impl Verifiable for AlpenglowState {
    fn verify(&self) -> AlpenglowResult<()> {
        // Combined verification: safety, liveness, and byzantine resilience
        self.verify_safety()?;
...
impl TlaCompatible for AlpenglowState {
    fn to_tla_string(&self) -> String {
        serde_json::to_string(&self.export_tla_state()).unwrap_or_else(|_| "{}".to_string())
    }
    ...
    fn import_tla_state(&mut self, state: &Self) -> AlpenglowResult<()> {
        // Import state by copying fields from provided AlpenglowState reference.
        // This avoids JSON parsing and matches the trait signature.
        self.system_state = state.system_state.clone();
        self.component_health = state.component_health.clone();
        self.interaction_log = state.interaction_log.clone();
        self.performance_metrics = state.performance_metrics.clone();
        self.integration_errors = state.integration_errors.clone();
        self.global_clock = state.global_clock;
        self.benchmark_start_time = state.benchmark_start_time;
        self.tla_state_cache = state.tla_state_cache.clone();

        // Import component states
        self.votor_state = state.votor_state.clone();
        self.rotor_state = state.rotor_state.clone();
        self.network_state = state.network_state.clone();
...
mod tests {
    ...
    fn test_failure_detection() {
        ...
        assert!(failures.contains(&"votor_excessive_views".to_string()) || failures.contains(&"votor_excessive_view_changes".to_string()));
        assert!(state.component_health.get("votor").is_some());
    }
